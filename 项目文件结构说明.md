...
- `background/phone-verification-flow.js`：手机号验证共享流程模块，负责步骤 2 / 4 的手机号注册取号、提交手机号、轮询短信验证码与完成激活，也负责 OAuth 链路命中 `add-phone / phone-verification` 页面后的 HeroSMS 取号、复用、重发和换号。
...
- `background/verification-flow.js`：邮箱验证码共享流程层，封装重发、轮询、提交、失败回退、自定义邮箱跳过、共享验证码自动重发次数配置以及 2925 长轮询参数；当前验证码提交重试上限为 15 次，2925 每次重发验证码之间都会固定跑完一轮 15 次邮箱刷新轮询；若 `mail2925Mode = receive`，会额外把目标注册邮箱传给 2925 内容脚本做弱匹配；若邮箱验证码提交后页面转入 `add-phone / 手机号页`，则会把“后续需要手机号验证”的结果继续传给步骤 9，而不是误判为普通失败；若邮箱验证码提交后通信中断但认证页已进入 OAuth 同意页或手机号验证页，会按真实页面状态继续后续流程；若 2925 轮询命中“子邮箱已达上限邮箱”，会转入 2925 会话模块执行“记录时间、禁用 24 小时、切下一个账号并重新登录”，然后直接结束当前尝试。
...
- `background/steps/fetch-login-code.js`：步骤 8 实现，负责 `add-email` 提交、邮箱验证码阶段的邮箱轮询、验证码回填与回退控制；只会在真实 `/add-email` 或邮箱验证码页继续，避免把普通登录弹窗里的邮箱输入框误判为添加邮箱页；对非 2925 provider 会固定当前验证码页显示邮箱作为本次 Step 8 的目标邮箱；当 provider 为 2925 时，会在轮询前先确保当前 2925 账号已自动登录。
- `background/steps/fetch-signup-code.js`：旧邮箱注册验证码兼容步骤实现，负责注册邮箱验证码阶段的页面准备与验证码流程入口；当前普通 / Plus 主链 Step 4 已切换到手机短信验证码。
- `background/steps/fetch-signup-phone-code.js`：步骤 4 实现，负责注册手机短信验证码阶段的 HeroSMS 轮询、重发、验证码提交与激活记录收尾。
...
- `background/steps/oauth-login.js`：步骤 7 实现，负责刷新 OAuth 链接、优先点击手机号登录入口、提交步骤 2 的手机号、填写步骤 3 的密码，并确保进入真实 `add-email`、邮箱验证码页或 OAuth 同意页；普通可恢复登录态失败会按上限重试，但一旦识别到认证流程进入 `add-phone / 手机号页`，会立即退出步骤 7 内部重试。
...
- `background/steps/submit-signup-phone.js`：步骤 2 实现，负责注册入口点击、切换到手机号注册、通过 HeroSMS 获取手机号、提交手机号并等待密码页。
...
- `content/signup-page.js`：注册、登录、授权主内容脚本，负责 OpenAI / ChatGPT 页面上的步骤执行；当前 Step 2 会点击注册入口、切换到手机号模式、选择国家并提交手机号；Step 7 会优先点击“继续使用手机登录”、提交注册手机号、填写密码，并只在真实 `/add-email` 页面才允许 Step 8 输入邮箱；登录链路还会显式识别 `phone-verification` 页面，避免把手机验证码页误判成邮箱验证码页或普通未知页。
...
- `sidepanel/sidepanel.html`：侧边栏页面结构；当前步骤列表已改为动态容器，日志区提供“记录”按钮并挂接邮箱记录覆盖层，顶部新增“贡
  ...
  展示 `OAUTH / 回调 / 总状态` 三块真实运行态信息，同时把“接码”开关与“验证码重发”拆成同一行展示；HeroSMS 平台、国家与 API Key 行保
  持可配置，手机号优先注册链路会直接使用这组配置，`接码` 开关只控制 OAuth 后半段额外手机号验证是否自动续跑；页面继续加载 `managed-alias-utils.js` 与 `mail-provider-utils.js`，并把旧的“邮箱前缀”字段语义改为“别
...
- `sidepanel/sidepanel.js`：侧边栏主入口脚本，负责 UI 状态同步、动态步骤渲染、按钮交互、共享验证码自动重发次数配置与广播接收，并装
  ...
  会在 sidepanel 初始化时拉取并恢复到本地保存的国家/API 设置，手机号优先注册始终可读取这组配置，接码开关只影响 OAuth 后半段额外手机号验证；账号记录快照同步改为默认自动模式，只
...
- `tests/background-step7-recovery.test.js`：测试步骤 8 添加邮箱/获取邮箱验证码后直接提交（不再回放步骤 7），并为 2925 关闭重发间隔。
