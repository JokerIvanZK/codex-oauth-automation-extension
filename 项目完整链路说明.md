...
- LuckMail 邮箱在写入当前运行态、继续复用或轮询验证码前，必须先调用 `/api/v1/openapi/email/token/{token}/alive` 检查 token 对应邮箱是否还能正常拉取邮件列表。
- 当前运行态邮箱如果可用性检测失败，会清空当前 LuckMail 运行态并重新选择可复用邮箱或重新购买邮箱。
- 新购邮箱投入注册流程前会先做可用性检测；检测失败时默认持续重新购买，直到拿到可用邮箱或用户停止流程，避免已退款或无法读取邮件的邮箱进入后续步骤。
- LuckMail API 配置、保留标签缓存和本地“已用”标记必须写入 `chrome.storage.local` 持久配置；侧边栏或扩展重新打开后仍以持久化的 `luckmailUsedPurchases` 计算已用、未用和可复用状态。
