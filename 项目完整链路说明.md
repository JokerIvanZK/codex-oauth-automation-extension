...
- 通过 HeroSMS 申请注册手机号
- 提交手机号、填写密码并轮询手机短信验证码
- 填写姓名和生日
- 清理登录环境并刷新 OAuth 链接
- 使用刚注册的手机号登录 OAuth
- 登录后进入 `add-email`，生成或选取注册邮箱
- 轮询邮箱验证码并完成邮箱绑定
- 自动确认 OAuth 同意页
...
- 展示 HeroSMS 的接码国家与 API Key 设置；手机号优先注册链路会直接使用这组配置，`接码` 开关只控制 OAuth 后半段额外命中手机号验证页时是否自动续跑
- 展示 `Plus 模式` 开关与 PayPal 账号/密码配置；开启后步骤列表切换为 Plus 模式 13 步定义，普通模式的 Cookie 清理步骤不再显示或执行，添加邮箱并验证步骤会移动到 Plus 可见第 11 步
...
- 步骤文件名靠语义
- 新增步骤时不需要重命名后续文件
- 普通模式使用 10 步定义；Plus 模式使用 13 步定义，其中 Plus 可见步骤 10/11/12/13 复用 OAuth 手机号登录、添加邮箱并验证、OAuth 同意页与平台回调验证执行器，但按 Plus 可见步骤编号记录状态
...
- OAuth 链接
- 当前邮箱 / 注册手机号 / 密码
- 第 8 步固定的邮箱验证码页显示邮箱 `step8VerificationTargetEmail`
- 当前手机号验证激活记录 `currentPhoneActivation`，既服务于步骤 2 / 4 的手机号注册，也服务于步骤 9 中 OAuth 额外手机号验证
...
- `content/mail-2925.js` 会把“当前验证码步骤启动会话”对应的已试验证码写入 `chrome.storage.session`，键为 `seen2925CodeState`；同一个 Step 8 会话里试过的验证码不会再次提交，新会话开始时会重置。
...
- [background/steps/submit-signup-phone.js](c:/Users/projectf/Downloads/codex注册扩展/background/steps/submit-signup-phone.js)
- [background/phone-verification-flow.js](c:/Users/projectf/Downloads/codex注册扩展/background/phone-verification-flow.js)
- [content/signup-page.js](c:/Users/projectf/Downloads/codex注册扩展/content/signup-page.js)

流程：

1. 打开或复用注册页
2. 如果官网首页还没有进入认证弹窗，内容脚本会点击 `免费注册 / Sign up / Register / 创建账户`
3. 如果认证弹窗当前停留在邮箱输入模式，会点击 `继续使用手机登录 / Continue with phone` 一类入口切换到手机号模式
4. 后台通过 HeroSMS 申请当前国家的 OpenAI 手机号，并写入 `currentPhoneActivation` 与 `signupPhoneNumber`
5. 内容脚本按页面国家选择框匹配 HeroSMS 国家，再把号码转换为当前国家的本地号码格式后填写
6. 点击继续并等待进入 `create-account/password` 密码页
7. 标记 Step 2 完成，后续 Step 3 使用同一个认证页继续填写密码
...
1. 生成或读取密码
2. 更新运行态密码
3. 记录账号快照；手机号注册阶段可能还没有邮箱，因此记录允许先保存 `phoneNumber`
4. 让内容脚本填写密码
...
- [background/steps/fetch-signup-phone-code.js](c:/Users/projectf/Downloads/codex注册扩展/background/steps/fetch-signup-phone-code.js)
- [background/phone-verification-flow.js](c:/Users/projectf/Downloads/codex注册扩展/background/phone-verification-flow.js)
- [content/signup-page.js](c:/Users/projectf/Downloads/codex注册扩展/content/signup-page.js)

流程：

1. 检查步骤 2 保存的 `currentPhoneActivation`
2. 通过 HeroSMS `getStatus / getStatusV2` 轮询短信验证码
3. 第一段 60 秒没有短信时，后台会请求额外短信并点击页面上的重发按钮
4. 第二段 60 秒仍没有短信时，步骤 4 报错；Auto 会回到步骤 1，重新获取手机号开始本轮重试
5. 拿到短信验证码后，通过 `content/signup-page.js` 填入验证码
6. 如果页面明确拒绝验证码，会在提交上限内请求新短信并重提
7. 验证通过后完成 HeroSMS 激活记录，清理 `currentPhoneActivation`，必要时把号码保存为可复用号码
8. 进入资料页后继续 Step 5

补充行为：

- Step 4 不再走邮箱 provider，也不会在本阶段生成或消耗注册邮箱
- `verificationResendCount` 仍保留给邮箱验证码和 OAuth 后半段手机号验证；注册手机验证码当前按两段 60 秒窗口处理
- 手动点击 Step 4 前必须确保步骤 2 的认证页和手机号激活记录仍存在，否则会提示重新执行步骤 2
- 旧的 [background/steps/fetch-signup-code.js](c:/Users/projectf/Downloads/codex注册扩展/background/steps/fetch-signup-code.js) 仍保留为兼容模块，但当前普通 / Plus 步骤定义已经不再把它作为主链 Step 4
...
2. 打开最新 OAuth 链接
3. 优先点击 `继续使用手机登录 / Continue with phone`，使用步骤 2 保存的 `signupPhoneNumber` 填写手机号
4. 进入 `https://auth.openai.com/log-in/password` 后，填写步骤 3 保存的密码并点击继续
5. 如果登录后进入 `https://auth.openai.com/add-email`，则 Step 7 判定完成，把“添加邮箱并收码”交给 Step 8
6. 如果登录后直接进入邮箱验证码页或 OAuth 同意页，也会按真实页面状态继续后续步骤
7. 如果未进入目标页面，则按可恢复逻辑最多重试 3 次；但一旦当前失败已经明确进入 `https://auth.openai.com/add-phone`，则立即退出步骤 7 内部重试，不再继续第 2 / 3 次尝试
...
1. 先读取当前认证页状态
2. 只有当页面已经是真实 `add-email` 或邮箱验证码页时，Step 8 才会继续；普通登录弹窗里的“电子邮件地址”输入框不能被当成 `add-email`
3. 如果当前在 `https://auth.openai.com/add-email`，先调用邮箱生成链路解析本轮邮箱并提交
4. 进入 `https://auth.openai.com/email-verification` 后，对非 2925 provider 固定当前页面显示邮箱为本次 Step 8 的目标邮箱
5. 打开邮箱页或 API 轮询入口
6. 轮询邮箱验证码
7. 回填邮箱验证码
8. 如遇邮箱轮询类失败或显式的 `STEP8_RESTART_STEP7` 恢复错误，则按有限次数回到 Step 7 重试
9. 获取到邮箱验证码后直接在当前验证码页提交并继续进入 Step 9
...
- 如果 Step 8 发现当前仍在登录入口、手机号输入页或密码页，会报错并由 Auto 回到 Step 7 重新走手机号登录，而不是直接在登录弹窗里输入邮箱。
- 当邮箱验证码提交后进入 `phone-verification` 页时，内容脚本会显式把该页面识别为“手机验证码页”，避免与邮箱验证码页混淆。
...
Plus 模式通过 `plusModeEnabled` 开启，目标是在普通注册资料完成后，不执行原 Step 6 Cookie 清理，而是先完成 Plus Checkout 与 PayPal 授权，再复用现有 OAuth 后半段的手机号登录、添加邮箱、OAuth 同意页与平台验证链路。

Plus 模式可见步骤：

1. 第 1~5 步：沿用普通手机号注册、密码、手机验证码、资料填写链路。
...
6. 第 10 步：复用原 Step 7 OAuth 手机号登录执行器，但状态和日志按 Plus 可见第 10 步记录。
7. 第 11 步：复用原 Step 8 添加邮箱并验证执行器，但状态和日志按 Plus 可见第 11 步记录。
...
- 原 Step 8 `添加邮箱并验证`：Plus 模式下移动到可见第 11 步，继续负责 add-email 与邮箱验证码页的收码与提交。
...
8. 这条链路只影响“注册邮箱分配”；Step 8 仍然走 `custom` provider 既有的手动验证码确认逻辑
...
- 内容脚本来源统一归类到 `mail-163`，这样 Step 8 继续复用同一套验证码读取与邮件清理逻辑。
...
4. 手动点击 `登录` 或自动流程进入 Step 8 前，后台会先打开当前 2925 邮箱页，并读取页面顶部当前邮箱地址：如果仍停留在收件箱且顶部邮箱与目标邮箱一致，则直接复用；如果顶部邮箱不一致且启用了号池模式，则先清理 cookie 后登录当前选中的账号；如果顶部邮箱不一致且未启用号池模式，则直接调用现有停止逻辑结束流程；如果页面跳到登录页，则仍然只有号池模式开启时才自动登录
...
- `icloudTargetMailboxType = icloud-inbox` 时，Step 8 直接打开 iCloud Mail 收件箱轮询验证码。
- `icloudTargetMailboxType = forward-mailbox` 时，注册邮箱仍由 iCloud Hide My Email 生成，但 Step 8 改为打开 `icloudForwardMailProvider` 指定的转发目标邮箱收码；当前支持 `qq / 163 / 163-vip / 126 / gmail`。
...
5. 执行 `runAutoSequenceFromStep`
   ...
   - 步骤 8 若在邮箱验证码提交后进入 `add-phone / 手机号页`，会直接抛出 fatal 错误，不再先标记步骤成功
...
3. 侧边栏配置项
4. 动态邮箱生成逻辑
5. Step 4 手机短信验证码、Step 8 邮箱验证码与 Step 9 手机号验证流
6. 成功收尾逻辑
...
- 当前主链 Step 4 是手机短信验证码；旧邮箱注册验证码恢复逻辑仍保留在兼容模块中，不再作为普通 / Plus 主链入口。
...
- Step 7 在手机号登录后不会把普通登录弹窗里的邮箱输入框误判为 `add-email`；只有真实 `https://auth.openai.com/add-email`、邮箱验证码页或 OAuth 同意页才会进入后续步骤。
- Step 7 的收尾确认是主要责任边界；Step 8 默认建立在“手机号登录已完成并进入 add-email / 邮箱验证码页”的前提上，只在后台入口保留防御性回退判断，不替代 Step 7 收尾。
- Step 8 如果发现认证页已经进入登录超时报错/重试页，会直接报错并回到 Step 7 重新开始，而不是在 Step 8 内部点击 `重试`。
- Step 8 的重试页判定也覆盖 `/email-verification` 上的 `405 / Route Error`，避免这类页面被误当成普通未知页。
...
- `2925` 邮箱验证码链路会携带固定的步骤开始时间窗口，实际筛选下限为“步骤开始时间向前回看 10 分钟”；当前主链主要发生在 Step 8。
