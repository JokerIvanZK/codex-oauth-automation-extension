...
- 自动填写注册手机号、密码、姓名与生日，并在 OAuth 登录后添加邮箱
- 支持自定义密码；留空时自动生成强密码
- 自动显示当前使用中的密码，便于后续保存
- 自动获取注册手机短信验证码与绑定邮箱验证码
- 支持 `Hotmail`：继续使用 `邮箱 + 客户端 ID + 刷新令牌（refresh token）`，并可在远程服务与本地助手两种模式间切换
- 支持 `2925`：新增多账号池、自动登录登出、Step 8 命中“子邮箱已达上限邮箱”后的 24 小时禁用与自动切号
...
- Step 8 会在真实 `add-email` 页面添加邮箱并完成邮箱验证码；Step 9 会自动寻找 OAuth 同意页的“继续”按钮并监听本地回调地址
...
6. 只有在 `号池` 开关开启时，自动流程执行到 Step 8 前才会自动检查 2925 登录态；如果未登录，会先清理登录 cookie、等待 `3 秒`，再打开登录页，并在页面打开后再等待 `3 秒`，然后使用当前账号自动登录；填写完账号密码后会额外等待 `1 秒` 再点击登录，点击后若 `40 秒`内仍未进入收件箱，则会判定当前登录失败
7. 当 Step 8 轮询邮箱时遇到“子邮箱已达上限邮箱”，扩展会记录当前时间；如果还有下一个可用账号，就禁用当前账号 24 小时并自动切换登录；如果没有下一个可用账号，或当前未启用号池模式，则会直接复用现有“手动暂停 / 停止”逻辑终止自动流程
...
- `2925` 通过侧边栏里的 2925 账号池选择账号，并在 Step 8 前自动校验网页邮箱登录态
...
- Auto 模式在生成 2925 别名邮箱前，会自动分配当前可用账号；若 Step 8 命中上限，会自动切到下一个账号
...
否则插件虽然能生成 `@你的域名` 邮箱，但验证码邮件最后没人接收，后面的 Step 8 还是会失败。
...
2. Step 2 点击注册，切换到手机号模式，通过 HeroSMS 取号并提交手机号
3. Step 3 填写本轮密码
4. Step 4 轮询注册手机短信验证码并提交
5. Step 5 填写姓名和生日
6. Step 6 清理登录环境
7. Step 7 刷新 OAuth 链接，点击 `继续使用手机登录`，提交步骤 2 的手机号，再在密码页提交步骤 3 的密码
8. Step 8 只在真实 `https://auth.openai.com/add-email` 或邮箱验证码页继续：先添加邮箱，再轮询并提交邮箱验证码
9. Step 9 / Step 10 完成 OAuth 同意页与平台回调验证

也就是说：

- 注册阶段不再先消耗邮箱；邮箱只在手机号登录完成后的 Step 8 添加
- 如果邮箱可自动获取，Step 8 会直接生成或分配本轮邮箱
- 如果邮箱自动获取失败，后台会按现有策略重试或等待手动补充邮箱
...
- 自动点击进入注册流程
- 点击 `继续使用手机登录 / Continue with phone`
- 通过 HeroSMS 获取当前国家的 OpenAI 手机号
- 自动选择国家、填写手机号
- 点击 `继续`
- 等待进入 `https://auth.openai.com/create-account/password` 后继续 Step 3

### Step 3: Fill Password

- 使用第 2 步已经确定好的手机号继续注册
- 使用自定义密码或自动生成密码
- 在密码页填写密码并提交注册表单
...
### Step 4: Get Signup SMS Code

根据步骤 2 保存的 HeroSMS 激活记录，轮询手机短信验证码并提交到注册页。

在 `Auto` 模式下，如果 Step 4 当前轮失败，后台会回到 Step 1 重新开始当前轮，并重新获取注册手机号。
如果注册认证重试页正文里出现 `user_already_exists`，则会直接判定“当前用户已存在”，不会点击 `重试`，而是立即结束当前轮；开启自动重试时会直接继续下一轮。

Step 4 不再打开邮箱，也不生成或消耗注册邮箱。
...
当前 Step 7 的完成标准不是“页面打开成功”，而是：

- 已刷新到最新 OAuth 链接
- 先点击 `继续使用手机登录`
- 已提交步骤 2 保存的手机号
- 已进入 `https://auth.openai.com/log-in/password` 并提交步骤 3 保存的密码
- 认证页已经真正进入 `add-email`、邮箱验证码页或 OAuth 授权页
- 普通登录弹窗里的“电子邮件地址”输入框不会被当成 `add-email`
...
- Step 8 不负责替代 Step 7 的手机号登录；它默认消费的是“已经由 Step 7 确认稳定进入”的 `add-email` 或邮箱验证码页，只在后台入口做防御性状态兜底

支持：

- 手机号 + 密码登录
- 直接落到邮箱验证码页或 OAuth 授权页时按真实页面状态继续
- 直到 `add-email`、邮箱验证码页或 OAuth 授权页就绪才算步骤完成

### Step 8: Add Email + Verify

Step 8 默认要求当前认证页已经处于真实 `add-email` 或邮箱验证码页。

它只负责：

- 如果当前在 `https://auth.openai.com/add-email`，生成或选取本轮邮箱并提交
- 进入 `https://auth.openai.com/email-verification` 后打开邮箱并轮询验证码
- 填写并提交邮箱验证码
...
如果 Step 8 发现当前仍在登录入口、手机号输入页或密码页，会报错并回到 Step 7 重新执行手机号登录，不会在普通登录弹窗里直接输入邮箱。
...
content/signup-page.js     ChatGPT 官网 + OpenAI 注册/登录页步骤：Step 1 / 2 / 3 / 5 / 7 / 8 / 9
...
- 直接在 `Email` 输入框中粘贴邮箱
- 手动点 `Step 8` 时，如果邮箱为空，脚本会先自动尝试按当前邮箱生成方式获取；失败后再改为手填
- Auto 暂停在邮箱阶段时，仍可手动粘贴邮箱后点击 `Continue`
